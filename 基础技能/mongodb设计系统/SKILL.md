---
name: mongodb设计系统
description: "mongodb 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 mongodb 启发的 UI、参考 mongodb 的设计语言或用户提到 mongodb 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 mongodb 的 UI、参考 mongodb 的设计语言、要求"mongodb 风格"的界面或正在采用 mongodb 视觉识别的项目时应用。
...
版本: alpha
名称: MongoDB 设计分析
描述: MongoDB 携带强烈的双模式视觉标识 — 深色深青色英雄条带有明亮 MongoDB 绿色（{colors.brand-green}）CTA，配以鲜明的白色文档表面。签名的绿色药丸按钮在产品、定价、学习和 AI 用例表面上 unmistakable。系统使用 Euclid Circular A 作为显示面，锚定 3 层定价比较（免费/Flex/专用），并在带有彩色类别标签的卡片网格中呈现广泛的课程目录。覆盖范围跨越主页、Atlas 产品页面、社区版、MongoDB University、AI 用例和定价。
