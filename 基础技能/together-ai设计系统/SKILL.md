---
name: together-ai设计系统
description: "together.ai 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 together.ai 启发的 UI、参考 together.ai 的设计语言或用户提到 together.ai 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 together.ai 的 UI、参考 together.ai 的设计语言、要求"together.ai 风格"的界面或正在采用 together.ai 视觉识别的项目时应用。

## 设计参考

---
版本: alpha
名称: Together AI 启发的设计分析
描述: Together AI 设计语言的启发性解释 — 一个 AI 基础设施平台，其表面在近黑色英雄条（带有三色橙色品红色长春花渐变作为唯一的品牌铬）和明亮白色研究/定价/文档条之间交替，由自定义显示无衬线和大写单字眉毛面编织在一起。
