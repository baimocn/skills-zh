---
name: playstation设计系统
description: "playstation 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 playstation 启发的 UI、参考 playstation 的设计语言或用户提到 playstation 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 playstation 的 UI、参考 playstation 的设计语言、要求"playstation 风格"的界面或正在采用 playstation 视觉识别的项目时应用。
...
名称: PlayStation 设计分析
描述: |
  一个三表面营销系统，围绕交替的黑色、白色和 PlayStation Blue 章节组织，像控制台启动预告片一样滚动过观众。每个部分都有单一的编辑目的 — 英雄摄影、控制台产品渲染、PS Plus 层标注、新闻条 — 每个都拥有三种全出血画布模式之一。对于游戏品牌来说，铬异常安静：明亮 PlayStation Blue（`#0070d1`）承载每个主要 CTA 作为完全圆角药丸，专有 SST 面以签名权重 300（轻）渲染显示文案，营造通风、优质的感觉，清晰的 8px 半径次要卡片系统在任一画布模式上承载产品信息。系统从不装饰 — 铬上没有渐变背景，没有大气网格，没有超出微弱部分分隔的投影。图像完成所有繁重工作：控制台魅力镜头、游戏关键艺术和 PS Plus 层插图占据每个部分的 60-90%，文案压缩到小编辑槽中。
