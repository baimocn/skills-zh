---
name: raycast设计系统
description: "raycast 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 raycast 启发的 UI、参考 raycast 的设计语言或用户提到 raycast 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 raycast 的 UI、参考 raycast 的设计语言、要求"raycast 风格"的界面或正在采用 raycast 视觉识别的项目时应用。
...
版本: alpha
名称: Raycast 设计分析
属于: 一个深色画布开发者工具系统，将营销页面视为扩展的产品截图 — 纯近黑色背景、命令调色板模型作为英雄、Inter 排版（开启 ss03 风格集）以及不打破墨水氛围的单个白色 CTA 药丸。铬读起来像 Raycast 自己的命令调色板 UI 放大到营销页面：单色深色表面，带有微弱的表面阶梯（#07080a → #0d0d0d → #101111）、卡片上紧凑的 6–10px 半径、#242728 中的细线 1px 边框以及罕见的饱和强调色（Hacker News 黄色、Slack 红色、Mac 绿色、信息蓝色）爆发，保留给产品瓷砖类别插图。签名视觉时刻是红色渐变英雄标志 — 三条对角红色条纹横跨主页顶部，像启动横幅 — 配以全出血产品 UI 截图，显示 Raycast 的实际命令调色板、商店和 AI 聊天表面。
描述: |
  Raycast 的营销系统读起来像扩展的产品截图。铬就是营销规模的产品内铬：纯近黑色画布、细线 1px 边框、命令调色板风格卡片、全站启用 ss03 风格集的 Inter 排版、白色 CTA 药丸以及保留给扩展和功能插图的一小组饱和类别强调色（黄色/红色/绿色/蓝色）。部分节奏慷慨（约 96px），但页面从不打破色调连续性 — 整个站点位于一个连续的深色模式中。
