---
name: theverge设计系统
description: "theverge 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 theverge 启发的 UI、参考 theverge 的设计语言或用户提到 theverge 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 theverge 的 UI、参考 theverge 的设计语言、要求"theverge 风格"的界面或正在采用 theverge 视觉识别的项目时应用。
...
The Verge 的 2024 年重新设计感觉就像有人将 Condé Nast 杂志连接到芯片调音板上。画布几乎是黑色的（`#131313`），标题由残酷重显示面（Manuka）构建，高达 107px，整个页面充满了酸薄荷 `#3cffd0` 和紫外线 `#5200ff`，它们的行为不太像品牌颜色，更像危险胶带。故事瓷砖不是安静的灰色卡片 — 它们是饱和的、全出血色块（黄色、粉色、橙色、蓝色、紫色），感觉像粘贴的狂欢传单排列成时间线。情绪是"开发者控制台遇见俱乐部之夜遇见科技小报"：足够严肃以报道国会听证会，足够响亮以评论合成器。
