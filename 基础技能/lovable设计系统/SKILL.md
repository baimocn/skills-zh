---
name: lovable设计系统
description: "lovable 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 lovable 启发的 UI、参考 lovable 的设计语言或用户提到 lovable 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 lovable 的 UI、参考 lovable 的设计语言、要求"lovable 风格"的界面或正在采用 lovable 视觉识别的项目时应用。
...
Lovable 的网站通过克制散发温暖。整个页面坐落在奶油色、羊皮纸色调的背景（`#f7f4ed`）上，立即将其与大多数开发者工具网站的冷白色惯例分开。这不是为了极简主义而极简主义 — 这是故意选择感觉平易近人，几乎像模拟，像一个精心制作的笔记本。近黑色文本（`#1c1c1c`）与这种温暖奶油色形成对比，创造了对眼睛舒适的对比度，同时保持清晰的可读性。
