---
name: opencode-ai设计系统
description: "opencode.ai 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 opencode.ai 启发的 UI、参考 opencode.ai 的设计语言或用户提到 opencode.ai 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 opencode.ai 的 UI、参考 opencode.ai 的设计语言、要求"opencode.ai 风格"的界面或正在采用 opencode.ai 视觉识别的项目时应用。
...
名称: OpenCode 设计分析
描述: |
  一个终端原生营销系统，完全在 Berkeley Mono 中渲染 — 页面上的每个字，从英雄标题到页脚细则，都是等宽的。页面本身读起来像手册页或静态站点 README：温暖奶油画布（`#fdfcfc`）、近黑色墨水（`#201d1d`）、少数交互元素的 4px 半径矩形以及用作项目符号的括号 `[+]`/`[-]` ASCII 标记。品牌唯一的"视觉时刻"是一个模拟 OpenCode TUI 本身的深色英雄卡 — 黑色背景、等宽终端输出、ASCII 管道字符以及渲染为块像素 ASCII 的标志。每个部分都作为奶油画布上的细线边框文本块存在，没有阴影、没有渐变、没有装饰图像，系统中任何地方都没有非等宽字符。
