---
name: slack设计系统
description: "slack 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 slack 启发的 UI、参考 slack 的设计语言或用户提到 slack 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 slack 的 UI、参考 slack 的设计语言、要求"slack 风格"的界面或正在采用 slack 视觉识别的项目时应用。
...
版本: alpha
名称: Slacc 启发的设计分析
描述: Slacc 设计语言的启发性解释 — 一个建立在深茄子色主要颜色之上的工作场所消息品牌，带有奶油薰衣草英雄渐变、蓝色内联链接和药丸 CTA。系统将专有人文主义无衬线用于显示，配以单独的实用无衬线用于正文，并将产品 UI 模型放置在柔和粉彩网格英雄复合体内，既作为装饰又作为功能解释。
