---
name: uber设计系统
description: "uber 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 uber 启发的 UI、参考 uber 的设计语言或用户提到 uber 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 uber 的 UI、参考 uber 的设计语言、要求"uber 风格"的界面或正在采用 uber 视觉识别的项目时应用。
...
版本: alpha
名称: Uber 启发的设计分析
描述: Uber 设计语言的启发性解释 — 一个交通和配送超级应用品牌，其 Web 表面是黑白二重奏，由自定义几何显示无衬线框住，每个交互元素上都有单一签名药丸形状（半径 999px）强调，仅由骑手、司机和城市物体的编辑 4:3 插图装饰。
