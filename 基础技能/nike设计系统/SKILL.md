---
name: nike设计系统
description: "nike 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 nike 启发的 UI、参考 nike 的设计语言或用户提到 nike 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 nike 的 UI、参考 nike 的设计语言、要求"nike 风格"的界面或正在采用 nike 视觉识别的项目时应用。
...
名称: Nike 设计分析
描述: |
  一个摄影优先的商务系统，建立在极端排版对比之上 — 高耸的大写 Futura 显示锁定烧录到编辑活动图像中，坐落在密集、中性、近乎单色的零售铬之上，药丸形黑色 CTA、灰色搜索和标签药丸以及紧凑的 8px 网格产品卡片。品牌的声音是运动的、动态的和绝对的：纯黑色、纯白色、单一柔和表面灰色以及故意小的语义强调集（销售红、成功绿、克制类别色调）— 每个色度时刻都保留给编辑摄影或定价信号，从不装饰铬。
