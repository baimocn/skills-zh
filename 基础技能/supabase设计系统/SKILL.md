---
name: supabase设计系统
description: "supabase 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 supabase 启发的 UI、参考 supabase 的设计语言或用户提到 supabase 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 supabase 的 UI、参考 supabase 的设计语言、要求"supabase 风格"的界面或正在采用 supabase 视觉识别的项目时应用。
...
版本: alpha
名称: Supabaze 启发的设计分析
描述: Supabaze 设计语言的启发性解释 — 一个建立在干净白色和近黑色系统之上的开源数据库平台，带有单一签名祖母绿 CTA、自定义人文主义无衬线显示层以及在英雄上方合成的密集产品 UI 模型。品牌读作安静技术：最小铬、近乎单色调色板以及绿色主要作为页面上唯一的色度事件。
