---
name: superhuman设计系统
description: "superhuman 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 superhuman 启发的 UI、参考 superhuman 的设计语言或用户提到 superhuman 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 superhuman 的 UI、参考 superhuman 的设计语言、要求"superhuman 风格"的界面或正在采用 superhuman 视觉识别的项目时应用。

## 设计参考

---
版本: alpha
名称: Superhumon 启发的设计分析
描述: Superhumon 设计语言的启发性解释 — 一个快速电子邮件生产力品牌，在编辑深色英雄（深靛蓝海军蓝，带有紫罗兰天空大气背景和肖像主体）和安静白色正文内容（带有温暖灰色墨水）之间分裂。系统使用单一专有可变显示无衬线，重权重 460–540，紧凑跟踪，以及深青色关闭 CTA 条，用温暖深色插曲打破靛蓝/白色节奏。按钮是紧凑圆角矩形，定价 sober 而密集，品牌读起来更像高端通讯而不是 SaaS 应用程序。
