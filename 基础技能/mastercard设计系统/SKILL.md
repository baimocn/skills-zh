---
name: mastercard设计系统
description: "mastercard 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 mastercard 启发的 UI、参考 mastercard 的设计语言或用户提到 mastercard 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 mastercard 的 UI、参考 mastercard 的设计语言、要求"mastercard 风格"的界面或正在采用 mastercard 视觉识别的项目时应用。

## 设计参考

# 受 Mastercard 启发的设计系统

## 1. 视觉主题和氛围

Mastercard 的体验读起来像一本由柔和石头和信号橙色构建的温暖编辑杂志。画布是柔和的油灰奶油色（`#F3F0EE`）— 不是白色，不是灰色，而是感觉像优质年度报告纸张的颜色。在那个画布上，重要的东西都像体育场、药丸或完美的圆形。主导视觉手势是**超大半径**：英雄携带 40 点圆角，卡片完全药丸形，服务图像裁剪成圆形轨道，按钮要么完成药丸，要么舒适地适合 20 点。页面上几乎没有尖角。
