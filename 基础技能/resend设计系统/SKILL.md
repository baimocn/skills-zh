---
name: resend设计系统
description: "resend 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 resend 启发的 UI、参考 resend 的设计语言或用户提到 resend 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 resend 的 UI、参考 resend 的设计语言、要求"resend 风格"的界面或正在采用 resend 视觉识别的项目时应用。
...
名称: Resend 设计分析
描述: |
  Resend 的营销表面坐落在近纯黑色画布上，带有灰白色文本和单一签名颜色 — 深编辑衬线 Domaine
