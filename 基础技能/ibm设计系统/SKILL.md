---
name: ibm���ϵͳ
description: "ibm 设计系统参�?�?颜色、排版、间距、组件、布局模式。在构建�?ibm 启发�?UI、参�?ibm 的设计语言或用户提�?ibm 风格/外观/感觉时自动激活�?
license: MIT
---
...
## 何时使用此技�?
当用户想要构建看起来�?ibm �?UI、参�?ibm 的设计语言、要�?ibm 风格"的界面或正在采用 ibm 视觉识别的项目时应用�?
## 设计参�?
---
版本: alpha
名称: IBM 设计分析
描述: "一个忠实于 Carbon Design System 的企业营销画布：白色表面、木炭类型、IBM Blue�?0f62fe）作为单一自信强调色，以及故意的平角美学，圆角保持�?0�?px。字体运�?IBM Plex Sans，显示尺寸轻权重 300（品牌签名），正文和强调使用 400/600。卡片作为薄边框瓷砖存在，无阴影；部分通过微妙的灰色行分隔。铬是方形的，排版是轻的，系统中唯一的颜色是一个自信的蓝色 �?结果读作旧世界企业重力为云时代重新构架�?
...
- **Light-weight display type**: Plex Sans at weight 300 for 42�?6px headlines is the brand's typographic signature.
...
**`footer`** �?Charcoal footer (`{colors.inverse-canvas}`) with the IBM wordmark left and 5�? columns of caption-sized links. The only inverted surface above the page break.
