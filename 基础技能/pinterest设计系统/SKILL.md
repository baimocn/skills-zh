---
name: pinterest设计系统
description: "pinterest 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 pinterest 启发的 UI、参考 pinterest 的设计语言或用户提到 pinterest 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 pinterest 的 UI、参考 pinterest 的设计语言、要求"pinterest 风格"的界面或正在采用 pinterest 视觉识别的项目时应用。
...
名称: Pinterest 设计分析
描述: |
  一个摄影优先的发现系统，围绕 Pinterest Red CTA、砖石引脚网格和柔和温暖奶油铬组织，让图像通行。主页是一个穿着杂志出版商铬的内容发现工具：70px 显示标题、友好的 Pin Sans 排版、奶油色调中性调色板上的完全圆角药丸按钮（16px），以及锚定每个视口的粘性红色"注册" CTA。Pin 图像是系统的承重视觉元素 — 方形、肖像和风景引脚在基于列的砖石网格中平铺，每个瓷砖是完全圆角的 16px 半径卡片，由紧凑的 8px 装订线分隔。铬在其他方面安静：温暖灰色、真白色和单一饱和红色 — 没有装饰渐变，没有大气背景，没有超出柔和模态遮罩的阴影。
