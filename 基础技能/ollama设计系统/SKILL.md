---
name: ollama设计系统
description: "ollama 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 ollama 启发的 UI、参考 ollama 的设计语言或用户提到 ollama 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 ollama 的 UI、参考 ollama 的设计语言、要求"ollama 风格"的界面或正在采用 ollama 视觉识别的项目时应用。
...
名称: Ollama 设计分析
描述: |
  一个几乎叛逆的最小文档优先系统，将主页视为 Markdown README — 纸白色画布、36px 居中标题、单个黑色药丸 CTA、内联终端安装代码片段以及手绘骆驼吉祥物作为唯一的装饰元素。没有渐变，没有英雄摄影，没有营销烟火。铬是一个由纯黑色、纯白色和三种中性灰色组成的微型实用调色板；每个交互元素都完全圆角成药丸（`{rounded.full}`）；排版是 SF Pro Rounded 用于标题，配以系统无衬线用于正文，ui-monospace 用于代码。定价层、常见问题解答和"您的数据属于您"保证都位于薄边框卡片内的同一平面上 — 系统就是文档，文档就是系统。
