---
name: runwayml设计系统
description: "runwayml 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 runwayml 启发的 UI、参考 runwayml 的设计语言或用户提到 runwayml 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 runwayml 的 UI、参考 runwayml 的设计语言、要求"runwayml 风格"的界面或正在采用 runwayml 视觉识别的项目时应用。
...
Runway 的界面是作为网站实现的电影卷轴 — 深色、编辑、电影制作级设计，全出血摄影和视频是主要 UI 元素。这不是典型的技术产品页面；这是 AI 驱动创造力的视觉宣言。每个部分都感觉像电影中的一帧：戏剧性照明、广阔的景观以及在主导视口的高质量图像中捕捉的亲密人类时刻。
