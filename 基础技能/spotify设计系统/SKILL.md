---
name: spotify设计系统
description: "spotify 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 spotify 启发的 UI、参考 spotify 的设计语言或用户提到 spotify 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 spotify 的 UI、参考 spotify 的设计语言、要求"spotify 风格"的界面或正在采用 spotify 视觉识别的项目时应用。
...
Spotify 的 Web 界面是一个深色、沉浸式音乐播放器，将听众包裹在近黑色茧（`#121212`、`#181818`、`#1f1f1f`）中，专辑艺术和内容成为颜色的主要来源。设计理念是"内容优先的黑暗" — UI 退回到阴影中，以便音乐、播客和播放列表可以发光。每个表面都是木炭色的阴影，创造了一个类似剧院的环境，唯一的真实颜色来自标志性的 Spotify Green（`#1ed760`）和专辑艺术本身。
