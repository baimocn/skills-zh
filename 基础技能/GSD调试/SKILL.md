...
**Flags:**
- `--diagnose` 鈥?Diagnose only. Returns a Root Cause Report without applying a fix.

**Subcommands:** `list` 路 `status <slug>` 路 `continue <slug>`
</objective>

<available_agent_types>
Valid GSD subagent types (use exact names 鈥?do not fall back to 'general-purpose'):
- gsd-debug-session-manager 鈥?manages debug checkpoint/continuation loop in isolated context
- gsd-debugger 鈥?investigates bugs using scientific method
</available_agent_types>
