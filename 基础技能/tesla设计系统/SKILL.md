---
name: tesla设计系统
description: "tesla 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 tesla 启发的 UI、参考 tesla 的设计语言或用户提到 tesla 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 tesla 的 UI、参考 tesla 的设计语言、要求"tesla 风格"的界面或正在采用 tesla 视觉识别的项目时应用。
...
Tesla 的网站是激进减法的练习 — 一个产品就是一切而界面几乎是 nothing 的数字陈列室。页面以全视口英雄开始，用电影汽车摄影填充整个屏幕：三辆车排列在抛光混凝土上，背景是朦胧的城市景观天空，上面漂浮着半透明白色类型的单个模型名称。没有装饰边框，没有渐变，没有图案，没有阴影。UI 仅存在以提供足够的导航结构来让路。每个不是产品图像的像素都是空白空间，这种克制是设计系统最强大的声明。
