---
name: figma���ϵͳ
description: "figma 设计系统参�?�?颜色、排版、间距、组件、布局模式。在构建�?figma 启发�?UI、参�?figma 的设计语言或用户提�?figma 风格/外观/感觉时自动激活�?
license: MIT
---
...
## 何时使用此技�?
当用户想要构建看起来�?figma �?UI、参�?figma 的设计语言、要�?figma 风格"的界面或正在采用 figma 视觉识别的项目时应用�?
## 设计参�?
---
版本: alpha
名称: Figma 设计分析
描述: "一个自信的黑白编辑框架，被超大、手工剪裁的粉彩色块打断。营销画布严格单色 �?figmaSans 可变类型、纯白色表面、纯黑色墨水、药丸形 CTA �?而每个故事部分将页面放入饱和石灰、薰衣草、奶油、薄荷或粉色面板中，读起来像放在干净桌子上的便利贴。结果是一个既技术又快乐的设计系�?�?一个严肃工作的工具，由喜欢颜色的人制作�?
...
### Text
- **Ink** ({colors.ink}): All headline, body, and caption type on light surfaces. There is no softer mid-gray text role on marketing �?body copy is always black at weight 320�?40, and weight (not opacity) carries the hierarchy.
...
- **Tight line-heights on display, generous on body.** Display sizes run 1.00�?.10; body runs 1.40�?.45. The contrast reinforces that headlines are graphics and body copy is for reading.
