---
name: nvidia设计系统
description: "nvidia 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 nvidia 启发的 UI、参考 nvidia 的设计语言或用户提到 nvidia 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 nvidia 的 UI、参考 nvidia 的设计语言、要求"nvidia 风格"的界面或正在采用 nvidia 视觉识别的项目时应用。
...
名称: NVIDIA 设计分析
描述: |
  一个工程级营销系统，围绕两种表面模式组织 — 深黑色画布用于英雄和页脚章节，平纸白色画布用于正文内容 — 通过单一、几乎暴力饱和的 NVIDIA Green 强调连接，承载每个 CTA、每个活动标签以及标记卡片的小装饰角方块。系统毫不道歉地有棱角：每个表面上 2px 半径，NVIDIA 专有 EMEA 切口中的紧凑粗体无衬线排版，以及分离密集多列技术内容的细线灰色规则。没有装饰渐变，没有大气网格，没有柔和投影 — 只有黑色、白色、灰色和绿色堆叠成结构化的编辑网格，从产品卡片扩展到大规模行业着陆页面，而不会弯曲其规则。
