---
name: revolut设计系统
description: "revolut 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 revolut 启发的 UI、参考 revolut 的设计语言或用户提到 revolut 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 revolut 的 UI、参考 revolut 的设计语言、要求"revolut 风格"的界面或正在采用 revolut 视觉识别的项目时应用。
...
名称: Revolut 设计分析
描述: |
  Revolut 的营销表面将鲜明的黑色画布与品牌的钴紫色（`#494fdf`）以及广泛深沉、完全饱和的强调调色板配对
