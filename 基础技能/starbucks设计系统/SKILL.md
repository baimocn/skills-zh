---
name: starbucks设计系统
description: "starbucks 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 starbucks 启发的 UI、参考 starbucks 的设计语言或用户提到 starbucks 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 starbucks 的 UI、参考 starbucks 的设计语言、要求"starbucks 风格"的界面或正在采用 starbucks 视觉识别的项目时应用。
...
Starbucks 的设计系统是一个**温暖、自信的零售旗舰店**，将他们店面围裙的绿色穿在每个表面上。画布在中性温暖奶油色（`#f2f0eb`）和陶瓷灰白色（`#edebe9`）之间交替 — 这些颜色参考了实际商店材料：纸巾、咖啡馆墙壁、木饰面 — 而签名的 **Starbucks Green**（`#006241`）在英雄条、CTA 和奖励体验上锚定品牌时刻。绿色有四种校准色调（Starbucks、Accent、House、Uplift），每种都映射到特定的表面角色，金色（`#cba258`）仅出现在奖励状态仪式周围 — 不是作为一般强调色。
