---
name: notion设计系统
description: "notion 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 notion 启发的 UI、参考 notion 的设计语言或用户提到 notion 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 notion 的 UI、参考 notion 的设计语言、要求"notion 风格"的界面或正在采用 notion 视觉识别的项目时应用。
...
版本: alpha
名称: Notion 设计分析
描述: Notion 通过自信、插图丰富的品牌声音呈现为一体化工作空间 — 锚定在深海军蓝英雄条（{colors.brand-navy}）上，装饰有品牌颜色的便利贴点和网格线插图，签名紫色药丸主要 CTA（{colors.primary}），以及呼应实时产品彩色数据库属性的丰富粉彩色功能卡片。系统在每个 UI 表面上使用 Notion-Sans（基于 Inter）字体，锚定 4 层定价比较（免费/Plus/商业/企业），并直接在英雄条中呈现实时工作空间 UI 模型。覆盖范围跨越主页、企业、产品 AI、产品代理、初创公司和定价表面。
