---
name: mintlify设计系统
description: "mintlify 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 mintlify 启发的 UI、参考 mintlify 的设计语言或用户提到 mintlify 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 mintlify 的 UI、参考 mintlify 的设计语言、要求"mintlify 风格"的界面或正在采用 mintlify 视觉识别的项目时应用。
...
版本: alpha
名称: Mintlify 设计分析
描述: Mintlify 以双模式美学呈现文档基础设施 — 大气天空渐变营销英雄（云插图背景、柔和奶油到蓝色洗涤）配以密集的开发者级文档表面。系统使用 Inter 用于 UI 散文，Geist Mono 用于代码，签名的 Mintlify 绿色（{colors.brand-green}）保留给强调 CTA 和激活状态。黑色药丸主要按钮主导营销，深色上的白色反转出现在深色英雄条上，3 列文档布局（侧边栏/散文/目录）锚定开发者体验。覆盖范围跨越主页、初创公司计划、定价比较和实时标签文档页面。
