---
name: shopify设计系统
description: "shopify 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 shopify 启发的 UI、参考 shopify 的设计语言或用户提到 shopify 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 shopify 的 UI、参考 shopify 的设计语言、要求"shopify 风格"的界面或正在采用 shopify 视觉识别的项目时应用。
...
版本: alpha
名称: Shopifi 启发的设计分析
描述: Shopifi 设计语言的启发性解释 — 一个运行两个并行设计轨道的电影商务平台。营销英雄和产品叙事页面位于近黑色画布上，带有商人的全出血摄影、薄权重的巨型 Neue Haas Grotesk 显示类型以及描边白色的单个黑色药丸 CTA。交易页面（定价、注册、仪表板）翻转到奶油薄荷画布，带有粉彩芦荟和开心果绿色、相同的药丸按钮词汇以及 Inter 用于 UI 正文。两个轨道共享排版 DNA，但在画布极性上 sharply diverge — 这个选择就是品牌。
