---
name: sanity设计系统
description: "sanity 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 sanity 启发的 UI、参考 sanity 的设计语言或用户提到 sanity 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 sanity 的 UI、参考 sanity 的设计语言、要求"sanity 风格"的界面或正在采用 sanity 视觉识别的项目时应用。
...
Sanity 的网站是一个开发者内容平台，呈现为夜间指挥中心 — 深色、精确且深度结构化。整个体验坐落在近黑色画布（`#0b0b0b`）上，读起来不太像"深色模式切换"，更像为生活在终端中的人构建的工具的自然状态。当大多数 CMS 营销页面追求友好的粉彩和柔和插图时，Sanity 倾向于其自身产品的重力：结构化内容值得一个结构化舞台。
