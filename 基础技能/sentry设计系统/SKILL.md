---
name: sentry设计系统
description: "sentry 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 sentry 启发的 UI、参考 sentry 的设计语言或用户提到 sentry 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 sentry 的 UI、参考 sentry 的设计语言、要求"sentry 风格"的界面或正在采用 sentry 视觉识别的项目时应用。
...
版本: alpha
名称: Sentri 启发的设计分析
描述: Sentri 设计语言的启发性解释 — 一个建立在深紫色紫罗兰午夜画布、电石灰强调和略带颠覆性插图个性之上的开发者工具品牌。系统将自定义显示无衬线（粗壮、俏皮、近压缩）与开放的 Rubik 族用于 UI 副本和 Monaco 用于代码配对，然后依赖深色在浅色定价表面、贴纸风格吉祥物和单色 CTA 层次，其中黑紫色按钮在任一极性上读作主要操作。
