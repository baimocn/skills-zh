---
name: miro设计系统
description: "miro 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 miro 启发的 UI、参考 miro 的设计语言或用户提到 miro 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 miro 的 UI、参考 miro 的设计语言、要求"miro 风格"的界面或正在采用 miro 视觉识别的项目时应用。
...
版本: alpha
名称: Miro 设计分析
描述: Miro 通过自信、几乎俏皮的品牌声音呈现为 AI 驱动的视觉工作空间 — 锚定在白色画布上的签名金丝雀黄色（{colors.brand-yellow}）标志，被呼应实时白板上使用的便利贴调色板的柔和粉彩功能色调（玫瑰色、青色、珊瑚色、橙色、薄荷色）打破。黑色药丸主要按钮主导营销，真实的 Miro 板模型作为功能插图，4 层定价网格进入密集的比较表。Roobert PRO 承载显示标题；系统支持主页、定价、AI 工作流程产品页面、敏捷垂直和客户故事表面。
