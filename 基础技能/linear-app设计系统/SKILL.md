---
name: linear-app设计系统
description: "linear.app 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 linear.app 启发的 UI、参考 linear.app 的设计语言或用户提到 linear.app 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 linear.app 的 UI、参考 linear.app 的设计语言、要求"linear.app 风格"的界面或正在采用 linear.app 视觉识别的项目时应用。
...
版本: alpha
名称: Linear 设计分析
描述: "一个近黑色产品聚焦营销画布，围绕 #010102（此集合中任何工具最深的深色表面）、浅灰色文本（#f7f8f8）和签名 Linear 薰衣草蓝（#5e6ad2）构建，用作单一色度强调。系统读作软件工艺文档：密集、技术性和安静奢华。显示类型设置在 Linear 自定义无衬线（SF Pro Display 后备）中，权重 500–700，带有测量的负跟踪。卡片作为木炭面板（#0f1011）存在，带有细线边框。强调薰衣草色出现在品牌标志、焦点环和一些有意的 CTA 上 — 从不装饰性地。页面节奏依赖于在深色面板中框住的产品 UI 截图，而不是大气颜色。"
