---
name: meta设计系统
description: "meta 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 meta 启发的 UI、参考 meta 的设计语言或用户提到 meta 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 meta 的 UI、参考 meta 的设计语言、要求"meta 风格"的界面或正在采用 meta 视觉识别的项目时应用。
...
版本: alpha
名称: Meta 设计分析
描述: Meta 的设计系统跨越硬件商务（Quest VR、Ray-Ban Meta AI 眼镜）和品牌表面，具有自信的商品销售声音。系统将鲜明的白色画布与全出血产品摄影卡片、自信的 Optimistic VF 标志/标题面、双 CTA 英雄模式（黑色主要 + 轮廓次要）以及饱和钴蓝色（#0064E0）用于产品内购买操作配对。药丸形 100px 半径按钮、慷慨的 24-32px 卡片圆角和紧凑的三层文本层次贯穿主页、产品详情（PDP）、立即购买配置器和配件子页面。
