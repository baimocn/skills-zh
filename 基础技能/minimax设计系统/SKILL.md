---
name: minimax设计系统
description: "minimax 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 minimax 启发的 UI、参考 minimax 的设计语言或用户提到 minimax 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 minimax 的 UI、参考 minimax 的设计语言、要求"minimax 风格"的界面或正在采用 minimax 视觉识别的项目时应用。
...
版本: alpha
名称: MiniMax 设计分析
描述: MiniMax 通过引人注目的二元性呈现为优质 AI 基础设施品牌 — 大胆的黑色药丸 CTA 和鲜明的白色画布用于营销，配以充满活力的渐变产品卡片（橙红色、品红色粉色、紫色、蓝色），将每个模型发布变成独特的视觉标识。系统在所有表面上使用 DM Sans，使用超大 80px 英雄显示，在深近黑色药丸中锚定主要操作，并通过带有侧边栏导航、散文正文和目录的 3 列文档网格分层内容密度。覆盖范围跨越营销主页、模型展示页面、开发者文档和平台定价表面。
