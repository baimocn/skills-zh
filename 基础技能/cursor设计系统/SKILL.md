---
name: cursor���ϵͳ
description: "cursor 设计系统参�?�?颜色、排版、间距、组件、布局模式。在构建�?cursor 启发�?UI、参�?cursor 的设计语言或用户提�?cursor 风格/外观/感觉时自动激活�?
license: MIT
---
...
## 何时使用此技�?
当用户想要构建看起来�?cursor �?UI、参�?cursor 的设计语言、要�?cursor 风格"的界面或正在采用 cursor 视觉识别的项目时应用�?
## 设计参�?
---
版本: alpha
名称: Cursor 设计分析
描述: 一�?AI 优先的代码编辑器，其营销网站读起来像一个安静自信的开发工具品牌，带有温暖奶油画布（`#f7f7f4`），而不是典型的深色 IDE 氛围。近黑色温暖墨水（`#26251e`）承载正文和显示 �?显示位于权重 400，带有负字母间距，感觉像杂志而不是粗体技术声音。单一品牌电压�?**Cursor Orange**（`#f54e00`），保留给主�?CTA 和标志。签名的柔和时间线调色板（桃色、薄荷色、蓝色、薰衣草色、金色）标记 AI 操作阶段（思�?阅读/编辑/搜索/完成）�?仅在产品内时间线可视化中。卡片使用最小细线、无阴影、慷慨的 80px 部分节奏。CursorGothic 用于显示/正文，JetBrains Mono 用于每个代码表面（大约占页面的一半）�?
colors:
  primary: "#f54e00"
...
|---|---|---|
| Mobile | < 640px | Hero h1 72�?2px; IDE mockup collapses to single pane preview; feature grid 1-up; nav hamburger. |
| Tablet | 640�?024px | Hero h1 56px; IDE mockup compresses; feature grid 2-up. |
| Desktop | 1024�?280px | Full hero h1 72px; full multi-pane IDE mockup; feature grid 3-up. |
