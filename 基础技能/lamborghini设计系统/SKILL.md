---
name: lamborghini���ϵͳ
description: "lamborghini 设计系统参�?�?颜色、排版、间距、组件、布局模式。在构建�?lamborghini 启发�?UI、参�?lamborghini 的设计语言或用户提�?lamborghini 风格/外观/感觉时自动激活�?
license: MIT
---
...
## 何时使用此技�?
当用户想要构建看起来�?lamborghini �?UI、参�?lamborghini 的设计语言、要�?lamborghini 风格"的界面或正在采用 lamborghini 视觉识别的项目时应用�?
## 设计参�?
# �?Lamborghini 启发的设计系�?
## 1. 视觉主题和氛�?
Lamborghini 的网站是一个黑暗大教堂 �?一个数字舞台，漆黑表面无限延伸，每个元素从虚空中出现，就像聚光灯下的机器。页面几乎完全是黑色的。不是深灰色，不是近黑色 �?真正的、不妥协的黑色（`#000000`），饱和视口并拒绝屈服。在这个深渊中，白色类型�?Lamborghini Gold（`#FFC000`）以外科手术般的精度部署，创造了一种视觉语言，感觉就像穿过夜间赛车活动，每个表面吸收光线，除了重要的东西�?
...
| Button Standard | 14.4px (0.90rem) | 300/700 | 1.00 | 0.14�?.2px | LamboType, uppercase, ghost buttons |
| Button Small | 13px (0.81rem) | 300/500 | 1.20 | 0.13�?.2px | LamboType, compact button variant |
| Caption | 14px (0.88rem) | 600/700 | 1.14�?.50 | -0.42px | LamboType, uppercase, negative tracking |
| Label | 12px (0.75rem) | 400/500 | 1.83 | 0.96px | LamboType, uppercase badges and micro labels |
| Micro | 10px (0.63rem) | 400 | 1.00�?.00 | 0.225px | LamboType, uppercase, smallest text |
...
- **Section padding**: 48�?6px vertical, 40px horizontal
- **Small spacing**: 2�?px for fine adjustments (badge padding, border spacing)
...
- Maintain tight line-heights (0.92�?.19) on display type to create dense, architectural text blocks
