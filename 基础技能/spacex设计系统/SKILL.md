---
name: spacex设计系统
description: "spacex 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 spacex 启发的 UI、参考 spacex 的设计语言或用户提到 spacex 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 spacex 的 UI、参考 spacex 的设计语言、要求"spacex 风格"的界面或正在采用 spacex 视觉识别的项目时应用。
...
版本: alpha
名称: Spacex 启发的设计分析
描述: Spasex 设计语言的启发性解释 — 一个建立在纯黑色画布上的任务导向航空航天品牌，火箭和火星景观的全出血摄影和视频英雄，以及大写 D-DIN 显示类型设置在紧凑的垂直行距中。UI 铬故意最小化 — 每个条带一个幽灵轮廓药丸按钮，大写眉毛微文本，以及摄影上的固定顶部导航。系统毫不道歉地朴素 — 黑色、白色和图像本身。
