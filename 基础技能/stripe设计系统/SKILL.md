---
name: stripe设计系统
description: "stripe 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 stripe 启发的 UI、参考 stripe 的设计语言或用户提到 stripe 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 stripe 的 UI、参考 stripe 的设计语言、要求"stripe 风格"的界面或正在采用 stripe 视觉识别的项目时应用。
...
版本: alpha
名称: Stripi 启发的设计分析
描述: Stripi 设计语言的启发性解释 — 一个建立在深海军蓝墨水、电靛蓝主要颜色以及占据几乎每个营销页面上三分之一的 recurring 大气渐变网格之上的金融基础设施品牌。系统将专有 Sohne 族在薄（300）权重与负字母间距配对，用于编辑密度显示标题，并在金钱和数字重要的地方使用表格图形正文类型。按钮是紧凑半径药丸，卡片位于近白色表面上，仪表板轨道翻转极性到熟悉的应用程序 shell。
