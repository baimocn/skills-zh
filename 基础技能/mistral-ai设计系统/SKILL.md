---
name: mistral-ai设计系统
description: "mistral.ai 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 mistral.ai 启发的 UI、参考 mistral.ai 的设计语言或用户提到 mistral.ai 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 mistral.ai 的 UI、参考 mistral.ai 的设计语言、要求"mistral.ai 风格"的界面或正在采用 mistral.ai 视觉识别的项目时应用。
...
版本: alpha
名称: Mistral AI 设计分析
描述: Mistral AI 以独特签名品牌化 — 大气日落渐变（芥末色、橙色、深红色）分层在山脉摄影上，加上关闭每个页面的水平"日落条"条。系统将温暖奶油黄色表面（{colors.cream}）与饱和橙色主要 CTA（{colors.primary}）配对，并为英雄显示使用优雅的近衬线声音。覆盖范围跨越主页（前沿 AI 英雄）、Le Studio 产品页面、编码解决方案、新闻文章表面、联系表单和服务层页面 — 全部锚定在签名渐变关闭条上。
