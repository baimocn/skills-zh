---
name: posthog设计系统
description: "posthog 设计系统参考 — 颜色、排版、间距、组件、布局模式。在构建受 posthog 启发的 UI、参考 posthog 的设计语言或用户提到 posthog 风格/外观/感觉时自动激活。"
license: MIT
---
...
当用户想要构建看起来像 posthog 的 UI、参考 posthog 的设计语言、要求"posthog 风格"的界面或正在采用 posthog 视觉识别的项目时应用。
...
名称: PostHog 设计分析
描述: |
  一个俏皮的开发者工具系统，渲染在温暖奶油画布上，手绘刺猬吉祥物像素描本中的旁注一样点缀在每个页面上。铬读起来像友好的工程博客：橄榄灰色墨水（#4d4f46）用于正文，深橄榄木炭色（#23251d）用于标题，IBM Plex Sans Variable 排版，紧凑的 1.43 行高段落，以及单一饱和黄橙色 CTA 药丸（#f7a501）承载每个主要操作。系统主动拒绝该类型典型的阴暗深色技术美学，转而支持奶油色、教科书插图感觉 — 带边框卡片堆叠在奶油画布上，带有 4–6px 半径，文档侧边栏使用圆角轮廓图标迷你插图，主页依靠卡通人物（穿实验室外套的刺猬、在终端前的刺猬、在躺椅上的刺猬）作为其签名装饰。代码示例和产品分析图表位于白奶油卡片内，带有薄橄榄边框；俏皮插图和数据密集产品图像之间的对比是品牌的声音。
